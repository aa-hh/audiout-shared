...
| `bt_sync:mic_permission_answered` | mac | `granted` | `true`, `false` | The alignment wizard asked for the microphone on Start and got its answer (the system prompt, or macOS's stored decision). Once per run; not sent when the wizard cannot listen at all (no probe, or both speakers make the same sound). |
| `bt_sync:listening_ended` | mac | `outcome`, `attempt`, `value_ms_bucket` | `outcome`: `measured` (a usable measurement became the proposal), `implausible` (a measurement landed but read as the speaker playing before the Mac; Try again listens once more), `failed` (no usable measurement; the by-ear questions follow); `attempt`: `1`, `2` — which listening pass of the run this was; `value_ms_bucket` (only on `measured` and `implausible`, omitted on `failed`): the measured value before clamping, as `below_-4`, `-4_to_0`, `0-9`, `10-39`, `40-99`, `100-199` or `200+` | The wizard's listening screen ends, for any reason other than the wizard closing. |
| `bt_sync:mic_retried` | mac | — | — | "Try again" is pressed on a proposal the microphone measured, and the wizard listens again. Only the first rejection of a measured proposal re-listens; the second reads "Align by ear" and is not this event. |
| `bt_sync:drift_corrected` | mac | `action`, `placement`, `magnitude_ms_bucket`, `surfaced` | `action`: `correct`, `swap_recorrect`; `placement`: `gap`, `slew`; `magnitude_ms_bucket`: `0-9`, `10-39`, `40-99`, `100+`; `surfaced`: `true`, `false` | The passive drift tracker moves a Bluetooth speaker's alignment after measuring it from the playing music. Fired after the move lands, never before: a `gap` move lands at once, a `slew` when its last step (or its in-gap remainder) lands. A slew replaced by a newer correction before it lands is not counted. No device names or ids. |
| `bt_sync:drift_window_ended` | mac | `result`, `trigger`, `refusal`, `error_ms_bucket`, `speaker_count`, `has_reference` | `result`: `aligned` (no Bluetooth speaker matched a peak), `observations`, `merged`, `rebaselined`, `unusable`, `blind`; `trigger`: `periodic`, `clock_jump`, `reconnect`, `audio_mode_change`, `silence_to_audio`, `verify`, `retry`; `refusal` (only when `result` is `unusable`): `reference_too_quiet`, `reference_too_narrowband`, `reference_too_periodic`, `slices_too_short`, `no_convincing_peak`; `error_ms_bucket` (only when `result` is `observations`; the largest per-speaker absolute error in the window): `0-9`, `10-39`, `40-99`, `100+`; `speaker_count`: number of Bluetooth speakers in the window; `has_reference`: `true` when an AirPlay or Cast anchor was part of the window, else `false` | The passive drift tracker finishes analysing one microphone window, fired beside the local `drift_window_result` line. No device names, ids or raw milliseconds. |
| `bt_sync:drift_window_skipped` | mac | `reason` | `reason`: `window_in_flight`, `blind`, `no_baselines`, `no_mic_permission`, `mic_start_failed` (the window never started); `empty_capture`, `no_mic_timestamp`, `reference_not_aligned` (the window was recorded but thrown away before analysis); `clock_step_storm` (a speaker's clock stepped often enough to be marked a bad link, so its steps take no windows) | The passive drift tracker could not take, or had to discard, a sampling window. One event per skip or drop. |
