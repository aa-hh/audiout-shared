...
| `bt_sync:listening_ended` | mac | `outcome`, `attempt` | `outcome`: `measured` (a usable measurement became the proposal), `implausible` (a measurement landed but read as the speaker playing before the Mac, so the run bowed out), `failed` (no usable measurement; a first pass that heard nothing listens once more, otherwise the by-ear questions follow); `attempt`: `1`, `2` — which listening pass of the run this was | The wizard's listening screen ends, for any reason other than the wizard closing. |
| `bt_sync:mic_retried` | mac | — | — | "Try again" is pressed on a proposal the microphone measured, and the wizard listens again. Only the first rejection of a measured proposal re-listens; the second reads "Align by ear" and is not this event. The automatic second listen after a failed first one is not this event either: it shows as `listening_ended` with `failed` and `attempt` `1`, followed by a second `listening_ended`. |
